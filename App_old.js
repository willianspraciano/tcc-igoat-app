...
export default function App() {
  ...
  useEffect(() => {
    async function loadModel(){
      ...
      const maskDetector = await tf.loadGraphModel(bundleResourceIO(modelJson,modelWeight))
        .catch(e => console.log(e));
      const mod = new automl.ObjectDetectionModel(maskDetector, ['background','Mucosa']); 
  ...
  const getFaces = async() => {
    try{
      ...
      const imageTensor = await imageToTensor(rawImageData).resizeBilinear([224,224]);

      const options = {score: 0.5, iou: 0.5, topk: 20};
      const predictions = await maskDetector.detect(imageTensor, options);
